...
namespace CleanSolution.Command.Services
{
    ...
    {
        private static readonly Regex _hasQuestionMarkRegEx = new(@"\?", RegexOptions.Compiled);

        private static readonly Regex _illegalCharactersRegex = new("[" + @"\/:<>|" + "\"]", RegexOptions.Compiled);

        private static readonly Regex _catchExtensionRegex = new(@"^\s*.+\.([^\.]+)\s*$", RegexOptions.Compiled);
        ...
        private Regex patternToRegEx(string pattern)
        {
            ...
            bool hasExtension = _catchExtensionRegex.IsMatch(pattern);
            bool matchExact = false;
            if (_hasQuestionMarkRegEx.IsMatch(pattern)) matchExact = true;
            else if (hasExtension) matchExact = _catchExtensionRegex.Match(pattern).Groups[1].Length != 3;
